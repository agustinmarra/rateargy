...
import { ExternalLink, Star, Check, Plus, Minus } from 'lucide-react'
...
// Gradiente de cada banco para simular la tarjeta física
const CARD_GRADIENT: Record<string, [string, string]> = {
  'ICBC Argentina':      ['#C8102E', '#7B0000'],
  'Banco Galicia':       ['#0072BC', '#003D7A'],
  'BBVA Argentina':      ['#004481', '#001E40'],
  'American Express':    ['#006FCF', '#003070'],
  'HSBC Argentina':      ['#DB0011', '#880000'],
  'Banco Macro':         ['#FFB81C', '#CC8800'],
  'Naranja X':           ['#FF6200', '#B34400'],
  'Santander Argentina': ['#EC0000', '#990000'],
  'Banco Provincia':     ['#006837', '#00381D'],
  'Ualá':               ['#7B2D8B', '#3D1645'],
  'Lemon Cash':          ['#F7B733', '#C4831A'],
}
const DEFAULT_GRADIENT: [string, string] = ['#1DB954', '#0E8A3A']
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-3">
        <div
          className="w-7 h-5 rounded-sm opacity-80"
          style={{
      ...
      {card.red && (
        <div className="absolute top-3 right-3">
          <span className="text-white/90 text-[8px] font-black tracking-wider">{card.red}</span>
        </div>
      )}
      {/* Nombre e info en el fondo */}
      <div className="absolute bottom-0 left-0 right-0 p-3">
        <div className="text-white/60 text-[8px] font-medium truncate">{card.banco}</div>
        <div className="text-white text-[10px] font-bold leading-tight truncate">{card.nombre}</div>
      </div>
    </div>
  )
}
    ...
    <div className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => (
        <Star
          key={i}
          size={12}
          className={i < filled ? 'fill-[#1DB954] text-[#1DB954]' : 'fill-[#e5e7eb] text-[#e5e7eb]'}
        />
      ))}
      <span className="text-xs text-[#6b7280] ml-1 font-medium">{score.toFixed(1)}/10</span>
    </div>
  )
}
  ...
  return (
    <div
      className={`relative bg-white rounded-xl border transition-all overflow-hidden ${
        isBestChoice
          ? 'border-[#1DB954] shadow-md shadow-[#1DB954]/10'
    ...
    >
      ...
      {isBestChoice && bestChoiceLabel && (
        <div className="absolute -top-px left-0 right-0 flex justify-start">
          <span className="bg-[#1DB954] text-white text-[10px] font-bold px-3 py-0.5 rounded-b-md ml-4">
            Nuestra elección para: {bestChoiceLabel}
          </span>
        </div>
      )}

      <div className={`flex flex-col lg:flex-row gap-4 p-5 ${isBestChoice ? 'pt-7' : ''}`}>
        ...
        <div className="flex-1 min-w-0 lg:min-w-[200px] lg:max-w-[220px]">
          <div className="flex items-start gap-2 mb-1">
            <BankLogo name={card.banco} size={32} />
            <div className="min-w-0">
              <div className="font-bold text-sm text-[#1a1a1a] leading-tight">{card.nombre}</div>
          ...
          {card.tag && (
            <span className="inline-block text-[10px] font-semibold px-2 py-0.5 rounded bg-[#1DB954]/10 text-[#15803d] mb-3">
              {card.tag}
            </span>
          )}
          ...
          <ul className="mt-3 space-y-1">
            {card.beneficios.slice(0, 3).map((b, i) => (
              <li key={i} className="flex items-start gap-1.5 text-xs text-[#6b7280]">
                <Check size={11} className="text-[#1DB954] shrink-0 mt-0.5" />
                {b}
              </li>
            ))}
          </ul>
        ...
        {/* ── Col 4: CTA ── */}
        <div className="flex flex-col items-center lg:items-end justify-between gap-3 shrink-0 lg:w-36">
          <div className="w-full">
            <a
              href={card.url_afiliado}
              target="_blank"
              rel="noopener noreferrer sponsored"
              className="flex items-center justify-center gap-1.5 w-full px-4 py-3 rounded-lg text-sm font-bold text-white bg-[#1DB954] hover:bg-[#17a349] transition-colors"
            >
              SOLICITAR AHORA
          ...
          <a
            ...
            className="text-xs text-[#1DB954] hover:text-[#17a349] transition-colors font-medium"
          >
            Ver condiciones →
          </a>
        </div>
